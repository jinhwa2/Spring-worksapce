...
@Controller // 요청하거나 응답제어 역할을 명시 Bean 등록 Bean =Spring에서 어떤 기능을 하라 설정해놓은 것
@RequestMapping("example") //example 시작하는 주소를 해당 컨트롤러에 넣어줌
...
public class ExampleController {
	 ...
	 * -@SessionAttributes 와 함께 사용할 경우 session scope반환
	 * [기본사용법]
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		//DTP 객체 Model을 이용해서 html 전달
		StudentDTO std = new StudentDTO();
		std.setStudentNo("12345");
		std.setStudentNo("홍길동");
		std.setAge(30);
		...
		/*
		 ...
		 <li th:text"${별칭}">mem 객체</li>
		   ...
		   */
		   ...
		   stdList.add(new StudentDTO("222","김이번",10));
		   stdList.add(new StudentDTO("333","김삼번",10));
		  ...
		  memList.add(new MemberDTO("200","최다이아","50"));
		  
		  model.addAttribute("memList", memList);
		  
		
		
		return "example/ex1"; //templates.example/ex.html 파일 바라보기
	}
	...
	public String ex2(Model model) {
		...
		model.addAttribute("str","<h1> 테스트중 &time: </h1>"); 
