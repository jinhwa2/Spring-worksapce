...
public class PigService {
	...
	public void uploadPig(String pig_name,int pig_age,MultipartFile file) {
		...
		//static바로 밑에 생성되기 떄문에 이미지가 보이지 않으므로 반드시 마지막에 / 를 넣어줘야함
		
		if(!imgFile.exists()    /*만약에 이미지 폴더가 존재하지 않을 때가 true이 !추가*/) {
			//폴더 생성하기
			imgFile.mkdir(); //폴더가 존재하지 않을 경우 우리가 지정한 폴더에 생성
		}
		...
		try {
			...
			//pig.setPig_image_path(uploaDir); //파일 이름만 저장
			//만약에 파일경로와 이름을 같이 저장하길 원한다면 아래 코드를 사용
			//pig.setPig_image_path("/images/" + uploadDir); //-> data에 /images/파일명.jpg
