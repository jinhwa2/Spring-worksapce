...
public class SCController {
	...
	// / (기본 주소) 가 index를 바라보기 떄문에 작성을 생략해도 되지만
	//특정한 값을 가져오거나 /(기본주소)가 index 이외에 다른 html파일을 바라볼 때는
	...
	public String getAllsc(Model model) {
		...
		/*
		 모든 db값 가져오는지 확인하기 [com.khmall.dto
	...
	//주소값으로 id 숫자를 가져와서 주소값에 지정된 id 숫자와 일치하는 db 값을 모두 가져오
	public String getSnackByID(Model model, @PathVariable int snack_id ) {
		//주소값으로 설정한 id를 받아오고 scservice에 전달해서 DB에서 꺼내고기 작업
	...
	@GetMapping("/company/{company_adderess}")
	public String getcompany(Model model, @PathVariable String company_adderess ) {
		//주소값으로 설정한 id를 받아오고 scservice에 전달해서 DB에서 꺼내고기 작업
		SC sc = scService.getcompany(company_adderess);
		model.addAttribute("company_adderess", sc);
