...
public class FileUploadUtil {
    public static void saveFile(String uploadDir, String fileName, MultipartFile mpFile) throws IOException {
    	...
    	//바탕화면에 imgFolder 생성하고 이미지 3개 선택해서 폴더에 저장 PostMan이용해서
    	Path uploadPath = Paths.get("C://Users//user1//Desktop" + uploadDir); //내가 지금 실행하고 있는 자바 파일 위치의 주소 가져오기
    	...
    	*/
        ...
        try( InputStream inputStream = mpFile.getInputStream() ){
        	...
        	// resolve = 해결하다 ㅡ 풀거나 결합 결단
        	// Path 계의 + 경로와 파일명을 붙여서 한번에 작성할 수 있게 도와주는것
        	...
        	//만약에 동일한 이름의 파일이 존재할 경오 덮어쓰기로 필수로 작성하는 것이 아님
