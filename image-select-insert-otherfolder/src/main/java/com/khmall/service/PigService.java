...
public class PigService {
	...
	/*
	 * @Autowired 를 사용해서 아래 코드를 모두 작성하지 않아도 됨 public PigService(PigMapper pigMapper)
	 * { this.pigMapper = pigMapper; } 서비스에서 추가적으로 수정된 데이터가 매퍼를 통해 db에 들어가고 db에서 가져온
	 * sql에 추가적으로 수정을 서비스단계에서 진행한다음 컨트롤러에 전달하거나 전달받기
	 */
	...
	public void uploadPig(String pig_name, int pig_age, MultipartFile file) {
		// 만약에 파일이름을 저장하길 원한다면 이미지 업로드 할 때 임시저장되어 있는 이미지 파일이름 가져옴
		...
		// 만약에 저장하고자 하는 이미지 폴더가 존재하지 않을 경우 경로들 모두 생성!
		// exist -> File 자료형에서만 생성
		if (!폴더경로확인.exists()) {
			폴더경로확인.mkdir(); // mkdir = 폴더 한개 mkdirs = 폴더 여러개
		...
		try {
			...
			System.out.println("Mapper에 Service로 pig insert 전달 성공");// -> 나중에 log로 전달
